...
static void test_get_move_input(void) {
    ...
    writeFixtureAndRedirect("phase_f_move_valid.txt", "E2 E4\n");
    ...
    writeFixtureAndRedirect("phase_f_move_compatible.txt", "  e2   E4  \n");
    assert(getMoveInput(&cmd) == 0);
    assert(cmd.type == CMD_MOVE);
    assert(positionEqual(cmd.from, createPosition(6, 4)) == 1);
    assert(positionEqual(cmd.to, createPosition(4, 4)) == 1);

    writeFixtureAndRedirect("phase_f_move_missing.txt", "E2\n");
    assert(getMoveInput(&cmd) != 0);
    assert(cmd.type == CMD_INVALID);

    writeFixtureAndRedirect("phase_f_move_invalid.txt", "Z9 E4\n");
    assert(getMoveInput(&cmd) != 0);
    assert(cmd.type == CMD_INVALID);
}
...
static void test_get_board_input(void) {
    ...
    writeFixtureAndRedirect("phase_f_board_valid.txt", "B7\n");
    assert(getBoardInput(&pos, &type) == 0);
    assert(positionEqual(pos, createPosition(1, 1)) == 1);
    assert(type == INPUT_SELECT_PIECE);

    writeFixtureAndRedirect("phase_f_board_invalid.txt", "B7 C6\n");
    assert(getBoardInput(&pos, &type) != 0);
    assert(type == INPUT_CANCEL);

    writeFixtureAndRedirect("phase_f_board_bad_coord.txt", "Z1\n");
    assert(getBoardInput(&pos, &type) != 0);
    assert(type == INPUT_CANCEL);
}
...
static void test_get_menu_selection(void) {
    ...
    writeFixtureAndRedirect("phase_f_menu_valid.txt", "2\n");
    assert(getMenuSelection(&selection) == 0);
    assert(selection == 2);

    writeFixtureAndRedirect("phase_f_menu_invalid.txt", "2 extra\n");
    assert(getMenuSelection(&selection) != 0);

    writeFixtureAndRedirect("phase_f_menu_not_number.txt", "abc\n");
    assert(getMenuSelection(&selection) != 0);
}
