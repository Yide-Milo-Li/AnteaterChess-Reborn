...
void gui_show_message_dialog(Gui *gui, GtkMessageType type, GtkButtonsType buttons, const char *title,
    ...
    dialog =
        gtk_message_dialog_new(parent, GTK_DIALOG_MODAL | GTK_DIALOG_DESTROY_WITH_PARENT, type, buttons, "%s", message);
    if (title != NULL) {
...
static void gui_on_confirm_response_clicked(GtkButton *button, gpointer user_data) {
    ...
    response = GPOINTER_TO_INT(g_object_get_data(G_OBJECT(button), "dialog-response"));
...
int gui_confirm(Gui *gui, const char *title, const char *message) {
    ...
    gtk_style_context_add_class(gtk_widget_get_style_context(body), "confirm-dialog-body");
    gtk_box_pack_start(GTK_BOX(contentArea), body, TRUE, TRUE, 0);

    icon = gui_create_ui_icon("icon-alert-dark.svg", 38);
    if (icon == NULL) {
        icon = gtk_label_new("!");
        gtk_style_context_add_class(gtk_widget_get_style_context(icon), "confirm-icon-fallback");
    }
    ...
    gtk_style_context_add_class(gtk_widget_get_style_context(titleLabel), "confirm-title");
    ...
    gtk_style_context_add_class(gtk_widget_get_style_context(messageLabel), "confirm-message");
    ...
    gtk_style_context_add_class(gtk_widget_get_style_context(buttonRow), "confirm-button-row");
    ...
    g_object_set_data(G_OBJECT(noButton), "dialog-response", GINT_TO_POINTER(GTK_RESPONSE_NO));
    g_signal_connect(noButton, "clicked", G_CALLBACK(gui_on_confirm_response_clicked), dialog);
    ...
    gtk_style_context_add_class(gtk_widget_get_style_context(yesButton), "destructive-button");
    g_object_set_data(G_OBJECT(yesButton), "dialog-response", GINT_TO_POINTER(GTK_RESPONSE_YES));
    g_signal_connect(yesButton, "clicked", G_CALLBACK(gui_on_confirm_response_clicked), dialog);
...
void gui_set_error(Gui *gui, AcErrorCode code) {
    const char *message = ac_get_error_message(code);

    if (code == AC_ERR_FATAL) {
...
void gui_rebuild_root_box(Gui *gui, GtkAlign halign, GtkAlign valign, int spacing) {
    ...
    gtk_style_context_add_class(gtk_widget_get_style_context(gui->main_box), "app-root");
