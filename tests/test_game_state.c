...
static void test_ai_time_budget_helpers(void) {
    ...
    assert(getDefaultAITimeBudgetMs(DIFFICULTY_TOURNAMENT) == 10000);
    ...
    config.initialTimeSeconds = 10;
    assert(getRequiredAITurnTimerSeconds(&config) == 11);
    assert(isAITurnTimerSettingValid(&config) == 0);
    config.initialTimeSeconds = 11;
    assert(isAITurnTimerSettingValid(&config) == 1);
