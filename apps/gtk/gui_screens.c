...
void gui_build_mode_menu(Gui *gui) {
    ...
    static const struct {
        ...
        {"Human vs. Human",       AC_MODE_HUMAN_VS_HUMAN      },
        {"Human vs. Computer",    AC_MODE_HUMAN_VS_COMPUTER   },
        {"Computer vs. Computer", AC_MODE_COMPUTER_VS_COMPUTER}
    };
...
void gui_build_endgame_menu(Gui *gui, const GuiView *state) {
    ...
    title = gtk_label_new(NULL);
    gtk_label_set_markup(GTK_LABEL(title), "<span size='x-large' weight='bold'>Game Over</span>");
    gtk_widget_set_halign(title, GTK_ALIGN_CENTER);
    gtk_box_pack_start(GTK_BOX(panel), title, FALSE, FALSE, 0);
...
void gui_build_screen_for_state(Gui *gui, const GuiView *state) {
    ...
    case AC_EXIT_STATE:
        ...
        break;
    case AC_INIT_STATE:
    case AC_GAME_TERMINATION_STATE:
    default:
        break;
    }
}
