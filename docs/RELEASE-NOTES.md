...
Breaking changes: all former public headers, C APIs and ABI are removed. Consult [Migration](MIGRATION.md). No new network mode, game-saving feature, or gameplay rule is introduced. Experimental uses Hard. Candidate capacity is 1,024 and chain length ten. macOS is outside first-release support. Validation evidence is tracked in [VALIDATION.md](VALIDATION.md); no unmeasured performance gain is claimed.
